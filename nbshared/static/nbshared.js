...
define([
    ...
    var Shared = function (unreviewed_selector, options) {
    ...
    Shared.prototype.bind_events = function () {
        var that = this;
        $('#refresh_shared_list').click(function () {
            that.load_list();
        });
    };


    Shared.prototype.load_list = function () {
        var settings = {
            processData : false,
            cache : false,
            type : "GET",
        ...
        var url = utils.url_join_encode(this.base_url, 'shared');
        $.ajax(url, settings);
    };


    Shared.prototype.clear_list = function () {
        // remove list items
    ...
    Shared.prototype.load_list_success = function (data, status, xhr) {
        this.clear_list();
        //console.log(data);
        var len = data.length;
        ...
        for (var i=0; i<len; i++) {
            var element = $('<div/>');
            var item = new NbShared(element, data[i], $.proxy(this.load_list_success, this), this.options);
            //console.log(element)
        ...
        // Add collapse arrows.
        $('.shared-notebooks-link').each(function(index, el) {
            var $link = $(el);
            var $icon = $('<i />')
    ...
    var NbShared = function (element, data, on_refresh, options) {
    ...
    NbShared.prototype.style = function () {
        this.element.addClass('list_item').addClass("row");
    };

    NbShared.prototype.escape_id = function () {
        ...
        var id = "nbgrader-" + this.data.course_id + "-" + this.data.notebook_id;
    ...
    NbShared.prototype.hash = function(s){
        return s.split("").reduce(function(a,b){a=((a<<5)-a)+b.charCodeAt(0);return a&a},0);
    }

    NbShared.prototype.make_row = function () {
        var row = $('<div/>').addClass('col-md-12');
        //var display_title = this.data.metadata.title || this.data.filename;
        var display_title = this.data.filename;
        ...
        if (this.data.owned) {
          /*
            btns.append($('<a/>')
                .attr("href", "shared/delete?notebook_id=" + this.data.filepath)
            ...
            btns.append($('<button/>')
                ...
                .click(function(){
                  var settings = {
                      url : utils.get_body_data("baseUrl")+"shared/delete?notebook_id=" + encodeURIComponent(that.data.filepath),
                      type : "GET",
                      success: function(data) {
                        $('#refresh_shared_list').trigger("click");
                      },
                      error: function(data) {
                        console.log("Impossible to delete the notebook");
                      }
                    }
                    $.ajax(settings);
                })
        ...
        btns.append($('<a/>')
            .attr("href", utils.get_body_data("baseUrl")+"shared/preview?notebook_id=" + encodeURIComponent(this.data.filepath))
        ...
        btns.append($('<button/>')
            ...
            .text('Use'));
        row.append(btns);
        row.append(this.make_modal(this.data.filepath, this.data.filename));
        ...
        if (this.data.status === 'fetched') {
            ...
            for (var i=0; i<this.data.notebooks.length; i++) {
                ...
                this.data.notebooks[i].notebook_id = this.data.notebook_id;
    ...
    NbShared.prototype.make_modal = function(notebook_id, filename) {
        var that = this;
        var modal_dialog;
        var modal_content;
        var modal_header;
        ...
        var container = $('<div/>')
            .addClass("modal")
            .addClass("fade")
            .attr("id", "modal-" + this.hash(notebook_id))
            .attr("tabindex", "-1")
            .attr("role", "dialog")
            .attr("aria-labelledby", "myModalLabel-" + this.hash(notebook_id));
        modal_dialog = $('<div/>')
        ...
        modal_title = $('<h4/>')
            ...
            .attr("id", "myModalLabel-" + this.hash(notebook_id))
            .text("Fetch a fresh copy to Notebooks/Shared Notebooks in your home directory")
        form = $('<form/>')
            .attr("action", utils.get_body_data("baseUrl")+"shared/fetch")
            .attr("method", "get")
            .attr("target", "_blank")
        modal_body = $('<div/>')
            .addClass("modal-body")
        ...
        dest_label = $('<label/>')
            ...
            .attr("for", "dest-" + this.hash(notebook_id))
            .text("Save Copy As")
        dest_input = $('<input/>')
            .attr("type", "text")
            .attr("name", "dest")
            .attr("value", filename)
            .attr("id", "dest-" + this.hash(notebook_id))
        hidden_input = $('<input/>')
            .attr("type", "hidden")
            .attr("name", "notebook_id")
            .attr("value", notebook_id)
    ...
    Notebook.prototype.make_row = function () {
        ...
        var url = utils.url_join_encode(this.base_url, 'tree', this.data.notebook_id, this.data.notebook_id) + ".ipynb";
